...
def api_connection():
    ...
    authorization = str(base64.b64encode(bytes(f':{pat}', 'ascii')), 'ascii')
    headers = {
...
def delete_agents(orgname,agent_pool_id,agent_name,agent_id):
    ...
    agent_delete_url = f"https://dev.azure.com/{orgname}/_apis/distributedtask/pools/{str(agent_pool_id)}/agents/{str(agent_id)}?api-version=6.0"
    resp2 = connection[0].request('DELETE', agent_delete_url, headers=connection[1])
    #print(resp2.data)
    print(f"Deleted Offline Agent---{agent_name}")
    #print(resp2.status)

# agents list
def list_agents(orgname,agent_pool_name):
    connection = api_connection()
    url = f"https://dev.azure.com/{orgname}/_apis/distributedtask/pools?poolName={agent_pool_name}&api-version=6.0"
    ...
    agent_list_url = f"https://dev.azure.com/{orgname}/_apis/distributedtask/pools/{str(pool_id)}/agents?api-version=6.0"
